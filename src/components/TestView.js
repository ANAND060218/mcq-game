...
const TEST_DURATION = 20; // seconds per question
...
const TestView = ({ questions, onComplete, onGoHome }) => {
    ...
    const finalizeAnswer = useCallback((answerToFinalize) => {
        if (isFinalized[currentQIndex]) return;

        const newAnswers = [...userAnswers];
        if (answerToFinalize === null) {
    ...
    }, [currentQIndex, isFinalized, isShortAnswer, question.isMultipleChoice, userAnswers]);
        ...
        const timer = setInterval(() => {
            setTimeRemaining(prevTimes => {
                ...
                if (newTimes[currentQIndex] > 0) {
                    ...
                    finalizeAnswer(currentSelection.length > 0 ? currentSelection : null);
                }
                return newTimes;
            });
        }, 1000);
    ...
    }, [currentQIndex, isFinalized, finalizeAnswer, currentSelection]);
    ...
    const handleSubmitTest = () => {
        const finalScore = questions.reduce((acc, q, index) => {
            ...
            if (q.answerOptions.length === 1) {
                ...
                isCorrect = String(userAnswer || '').toLowerCase() === q.correctAnswersList[0].toLowerCase();
    ...
    const getOptionClass = (optionText) => {
        if (!isFinalized[currentQIndex]) {
            return currentSelection.includes(optionText) ? 'selected' : '';
        }

        const isCorrectAnswer = question.correctAnswersList.includes(optionText);
        const wasSelected = userAnswers[currentQIndex] !== null && (Array.isArray(userAnswers[currentQIndex]) ? userAnswers[currentQIndex].includes(optionText) : userAnswers[currentQIndex] === optionText);
    ...
    const getShortAnswerClass = () => {
        if (!isFinalized[currentQIndex]) return '';
        const savedAnswer = userAnswers[currentQIndex];
        if (savedAnswer === null) return 'incorrect';
        
        // FIX: Ensure savedAnswer is treated as a string before .toLowerCase()
        const isCorrect = String(savedAnswer || '').toLowerCase() === question.correctAnswersList[0].toLowerCase();
        return isCorrect ? 'correct' : 'incorrect';
    };

    const renderAnswers = () => {
        const isAnswered = isFinalized[currentQIndex];
        const isWrongShortAnswer = isAnswered && getShortAnswerClass() === 'incorrect';
                ...
                <>
                    ...
                    {isWrongShortAnswer && (
                        <div className="solution-box">
                            ...
                            <p>{question.correctAnswersList[0]}</p>
                        </div>
                    )}
                </>
            );
                    ...
                    question.isMultipleChoice ? (
                        <div key={index} className={`checkbox-option ${getOptionClass(option.answerText)}`}>
                            <input
                                ...
                                checked={currentSelection.includes(option.answerText)}
                                onChange={() => handleSelection(option.answerText)}
                                disabled={isAnswered}
                            />
                            <label htmlFor={`q${currentQIndex}-opt${index}`}>{option.answerText}</label>
                        </div>
                    ) : (
                        <button key={index} className={`option-btn ${getOptionClass(option.answerText)}`} onClick={() => handleSelection(option.answerText)} disabled={isAnswered}>
